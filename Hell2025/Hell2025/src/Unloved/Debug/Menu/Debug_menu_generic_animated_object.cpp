...
namespace Debug::Menu::GenericAnimatedObjectMenu {
    ...
    const std::vector<Unloved::Bible::AnimationSlot> ARMS_HEAD_ANIMATION_SLOTS = {
        Unloved::Bible::AnimationSlot::FIRE_1,
        Unloved::Bible::AnimationSlot::FIRE_2,
        Unloved::Bible::AnimationSlot::FIRE_3,
        Unloved::Bible::AnimationSlot::RELOAD,
        Unloved::Bible::AnimationSlot::RELOAD_EMPTY
    };
    ...
    void BuildMenu() {
        ...
        for (Unloved::Bible::AnimationSlot animationSlot : ARMS_HEAD_ANIMATION_SLOTS) {
            armsHeadAnimationNames.push_back(Unloved::Bible::GetAnimation(Unloved::Bible::AnimationProfile::RAT_KING_GLOCK, animationSlot));
        }
        ...
        for (const Unloved::GenericAnimatedObject& genericAnimatedObject : Unloved::World::GetGenericAnimatedObjects()) {
            ...
            for (int32_t i = 0; i < static_cast<int32_t>(ARMS_HEAD_ANIMATION_SLOTS.size()); i++) {
                if (genericAnimatedObject.GetWeaponAnimationSlot() == ARMS_HEAD_ANIMATION_SLOTS[i]) {
    ...
    void ApplyEdit(uint32_t id, const Value& value) {
        ...
        if (id == static_cast<uint32_t>(Action::ARMS_HEAD_ANIMATION)) {
            ...
            // If selection wraps...
            if (value.intValue == 0) {
                genericAnimatedObject.SetWeaponAnimation(Unloved::Bible::AnimationSlot::UNDEFINED);
            }

            // Apply selection 
            if (value.intValue > 0) {
                genericAnimatedObject.SetWeaponAnimation(ARMS_HEAD_ANIMATION_SLOTS[value.intValue - 1]);
            }
        } 
