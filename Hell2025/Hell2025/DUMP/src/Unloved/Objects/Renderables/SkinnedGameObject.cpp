...
void SkinnedGameObject::SetSkinnedModel(const std::string& name, const std::string& presetName) {
...
void SkinnedGameObject::SetSkinnedModel(const std::string& name, SkinnedModelPreset preset) {
    SetSkinnedModelResource(name);
    Bible::ConfigureAnimatedMeshNodes(m_objectId, &m_animatedMeshNodes, preset);
}
...
uint64_t SkinnedGameObject::CreateRagdoll(const std::string& ragdollName, const PhysicsFilterData& filterData) {
    const RagdollAsset* asset = Hell::ResourceManager::GetRagdollAssetByName(ragdollName);
    return asset ? CreateRagdoll(*asset, filterData) : 0;
}

uint64_t SkinnedGameObject::CreateRagdoll(const RagdollAsset& asset, const PhysicsFilterData& filterData) {
    SetScale(asset.skinnedModelScale);
    m_ragdollId = Hell::Physics::SpawnRagdoll(m_transform.position, m_transform.rotation, asset, GetOwnerObjectId(), filterData);
    return m_ragdollId;
}
