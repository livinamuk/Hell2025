...
namespace Unloved::EditorSession::RagdollEditor {
    namespace {
        ...
        bool CreateSkinnedModelPreview() {
            ...
            if (!g_document.loaded || !g_document.showSkinnedModel || g_document.asset.skinnedModelName.empty()) return false;

            SkinnedModel* skinnedModel = Hell::ResourceManager::GetSkinnedModelByName(g_document.asset.skinnedModelName);
            if (!skinnedModel || skinnedModel->GetNodeCount() == 0 || skinnedModel->GetBoneCount() == 0) return false;
            ...
            if (g_document.asset.testMaterialPresetName.empty()) {
                skinnedGameObject->SetSkinnedModel(g_document.asset.skinnedModelName);
            }
            else {
                skinnedGameObject->SetSkinnedModel(g_document.asset.skinnedModelName, g_document.asset.testMaterialPresetName);
            }
        ...
        void SynchronizeSkinnedModelPreview() {
            if (!g_document.loaded || !g_document.showSkinnedModel || g_document.asset.skinnedModelName.empty()) {
                DestroySkinnedModelPreview();
                return;
            }

            SkinnedGameObject* skinnedGameObject = World::GetSkinnedGameObjectByObjectId(g_previewSkinnedGameObjectId);
            if (skinnedGameObject && skinnedGameObject->GetName() == SKINNED_MODEL_PREVIEW_NAME) return;
    ...
    void CleanUpSkinnedModelPreview() {
        DestroySkinnedModelPreview();
    }
    ...
    bool New(const std::string& name, const std::string& skinnedModelName, std::string& error) {
        ...
        if (!skinnedModel || skinnedModel->GetNodeCount() == 0 || skinnedModel->GetBoneCount() == 0) {
            error = "Select a skinned model for the new ragdoll";
            return false;
        }

        Document newDocument;
        newDocument.asset.name = name;
        newDocument.asset.skinnedModelName = skinnedModelName;
    ...
    bool Save(std::string& error) {
        ...
        if (g_document.asset.skinnedModelName.empty()) {
            error = "Select a skinned model before saving the ragdoll";
            return false;
        }
        SkinnedModel* skinnedModel = Hell::ResourceManager::GetSkinnedModelByName(g_document.asset.skinnedModelName);
        if (!skinnedModel || skinnedModel->GetNodeCount() == 0 || skinnedModel->GetBoneCount() == 0) {
            error = "Skinned model '" + g_document.asset.skinnedModelName + "' is not available";
            return false;
        }
    ...
    bool SetSkinnedModelName(const std::string& name) {
        if (!g_document.loaded) return false;

        if (!name.empty()) {
            SkinnedModel* skinnedModel = Hell::ResourceManager::GetSkinnedModelByName(name);
            if (!skinnedModel || skinnedModel->GetNodeCount() == 0 || skinnedModel->GetBoneCount() == 0) return false;
        }

        if (g_document.asset.skinnedModelName == name) return true;

        DestroySkinnedModelPreview();
        g_document.asset.skinnedModelName = name;
        ...
        ClearSelection();
        g_document.dirty = true;
        SynchronizeSkinnedModelPreview();
        return true;
    }
    ...
    bool SetTestMaterialPresetName(const std::string& name) {
        if (!g_document.loaded) return false;
        if (g_document.asset.testMaterialPresetName == name) return true;

        DestroySkinnedModelPreview();
        g_document.asset.testMaterialPresetName = name;
        g_document.dirty = true;
        SynchronizeSkinnedModelPreview();
        return true;
    }
    ...
    bool SetSkinnedModelVisible(bool visible) {
        if (!visible) {
            g_document.showSkinnedModel = false;
            DestroySkinnedModelPreview();
            return true;
        }

        if (!g_document.loaded || g_document.asset.skinnedModelName.empty()) return false;

        g_document.showSkinnedModel = true;
        if (CreateSkinnedModelPreview()) return true;
    ...
    std::vector<std::string> GetAvailableSkinnedModelNames() {
        ...
        for (auto& entry : Hell::ResourceManager::GetSkinnedModels()) {
            ...
            if (skinnedModel.GetName().empty() || skinnedModel.GetNodeCount() == 0 || skinnedModel.GetBoneCount() == 0) continue;
            names.push_back(skinnedModel.GetName());
        }
    ...
    const SkinnedModel* GetSkinnedModel() {
        if (!g_document.loaded || g_document.asset.skinnedModelName.empty()) return nullptr;
        return Hell::ResourceManager::GetSkinnedModelByName(g_document.asset.skinnedModelName);
    }
