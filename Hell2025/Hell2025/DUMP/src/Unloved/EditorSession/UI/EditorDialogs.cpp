...
namespace Unloved::EditorSession::FileDialog {
    namespace {
        ...
        constexpr uint64_t NEW_SKINNED_MODEL_INPUT_ID = UINT64_MAX - 3;
        ...
        std::string g_newSkinnedModelName;
        std::string g_pendingNewSkinnedModelName;
        std::string g_pendingRagdollSaveAsName;
        ...
        bool g_isSavingRagdollAs = false;
        bool g_isOpen = false;
        ...
        void Hide() {
            ...
            g_isSavingRagdollAs = false;
            g_files.clear();
            g_selectedFile.clear();
            g_newFileName.clear();
            g_newSkinnedModelName.clear();
            g_scrollBar = {};
        }
        ...
        void QueueNewFile() {
            ...
            if (Workspace::NameExists(g_mode, g_newFileName)) {
                const std::string assetType = g_isSavingRagdollAs ? "Ragdoll" : "Name";
                Dialog::Open(assetType + " '" + g_newFileName + "' already exists");
                return;
            }

            if (g_isSavingRagdollAs) {
                g_pendingRagdollSaveAsName = g_newFileName;
                Hide();
                return;
            }

            const bool requiresSkinnedModel = g_mode == EditorSessionMode::RAGDOLL || g_mode == EditorSessionMode::BONE_MASK;
            if (requiresSkinnedModel && (g_newSkinnedModelName.empty() || g_newSkinnedModelName == NO_SKINNED_MODEL)) {
                Dialog::Open("Select a skinned model");
                return;
            }
            g_pendingNewFileName = g_newFileName;
            g_pendingNewSkinnedModelName = requiresSkinnedModel ? g_newSkinnedModelName : std::string();
            Hide();
        }
        ...
        void RenderNewFileDialog(const EditorRect& canvasRect) {
            ...
            const bool requiresSkinnedModel = !g_isSavingRagdollAs && (g_mode == EditorSessionMode::RAGDOLL || g_mode == EditorSessionMode::BONE_MASK);
            const int32_t propertyRowCount = requiresSkinnedModel ? 2 : 1;
            const int32_t inputHeight = style.input.contentPadding * 2 + style.input.rowHeight * propertyRowCount;
            ...
            // Dialog window
            DrawDialogWindow(canvasRect, windowRect, g_isSavingRagdollAs ? "Save Ragdoll As" : "New File");
            UI::DrawBorder(windowRect, style.colors.border);
            ...
            if (requiresSkinnedModel) {
                modelNames = { NO_SKINNED_MODEL };
                std::vector<std::string> availableModelNames;
                if (g_mode == EditorSessionMode::RAGDOLL) availableModelNames = RagdollEditor::GetAvailableSkinnedModelNames();
                else availableModelNames = BoneMaskEditor::GetAvailableSkinnedModelNames();
                modelNames.insert(modelNames.end(), availableModelNames.begin(), availableModelNames.end());
                properties.DropDown(NEW_SKINNED_MODEL_INPUT_ID, "Skinned Model", modelNames, g_newSkinnedModelName);
            }
            ...
            const bool modelSelected = !requiresSkinnedModel || (!g_newSkinnedModelName.empty() && g_newSkinnedModelName != NO_SKINNED_MODEL);
            const bool createEnabled = !g_newFileName.empty() && modelSelected;
            ...
            DrawButton(createButtonRect, g_isSavingRagdollAs ? "Save" : "Create", createEnabled, createHovered);
    ...
    void New(EditorSessionMode mode) {
        Close();
        g_mode = mode;
        if (mode == EditorSessionMode::RAGDOLL || mode == EditorSessionMode::BONE_MASK) g_newSkinnedModelName = NO_SKINNED_MODEL;
    ...
    void Close() {
        ...
        g_pendingNewSkinnedModelName.clear();
        g_pendingRagdollSaveAsName.clear();
    }
    ...
    void Render() {
        ...
        const glm::uvec2 resolution = UIBackEnd::GetCanvasResolution(UICanvas::NATIVE);
        const EditorRect canvasRect = { 0, 0, static_cast<int32_t>(resolution.x), static_cast<int32_t>(resolution.y) };
        if (g_isCreatingNewFile || g_isSavingRagdollAs) {
    ...
    bool IsNameInputOpen() {
        return g_isOpen && (g_isCreatingNewFile || g_isSavingRagdollAs);
    }
    ...
    std::string ConsumeNewSkinnedModelName() {
        std::string modelName = std::move(g_pendingNewSkinnedModelName);
        g_pendingNewSkinnedModelName.clear();
        return modelName;
    }

    std::string ConsumeRagdollSaveAsName() {
        std::string name = std::move(g_pendingRagdollSaveAsName);
        g_pendingRagdollSaveAsName.clear();
        return name;
    }
}
