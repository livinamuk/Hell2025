...
void Player::FireGun() {
    ...
    AnimationSlot fireAnimationSlot = AnimationSlot::FIRE_1;
    AnimationSlot adsFireAnimationSlot = AnimationSlot::ADS_FIRE_1;
    if (randomAnimationIndex == 1) {
        fireAnimationSlot = AnimationSlot::FIRE_2;
        adsFireAnimationSlot = AnimationSlot::ADS_FIRE_2;
    }
    if (randomAnimationIndex == 2) {
        fireAnimationSlot = AnimationSlot::FIRE_3;
        adsFireAnimationSlot = AnimationSlot::ADS_FIRE_3;
    }
...
void Player::ReloadGun() {
    ...
    if (GetCurrentWeaponMagAmmo() == 0) {
        ...
        PlayViewWeaponAnimation(AnimationSlot::RELOAD_EMPTY, weaponInfo->animationSpeeds.reloadempty);
        m_weaponAction = RELOAD_FROM_EMPTY;
    }
    else {
        Audio::PlayAudio(weaponInfo->audioFiles.reload, 0.8f);
        PlayViewWeaponAnimation(AnimationSlot::RELOAD, weaponInfo->animationSpeeds.reload);
...
void Player::EnterADS() {
    WeaponInfo* weaponInfo = GetCurrentWeaponInfo();
    PlayViewWeaponAnimation(AnimationSlot::ADS_IN, weaponInfo->animationSpeeds.adsIn);
    m_weaponAction = ADS_IN;
}

void Player::LeaveADS() {
    WeaponInfo* weaponInfo = GetCurrentWeaponInfo();
    PlayViewWeaponAnimation(AnimationSlot::ADS_OUT, weaponInfo->animationSpeeds.adsOut);
