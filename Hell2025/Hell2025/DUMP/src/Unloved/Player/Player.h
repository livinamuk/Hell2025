...
public:
    ...
    void UpdateSkinnedGameObjects(float deltaTime);
    ...
    void PlayViewWeaponAnimation(AnimationSlot animationSlot, float speed);
    void PlayAndLoopViewWeaponAnimation(AnimationSlot animationSlot, float speed);

    // Character model animation
    void PlayAndLoopCharacterAnimation(const std::string& animationName, float speed);

    // Weapon logic
    void UpdateMeleeLogic(float deltaTime);
    void UpdateGunLogic(float deltaTime);
    void UpdateShotgunGunLogic(float deltaTime);
    void BeginMeleeAttack(AnimationSlot animationSlot);
...
private:
    ...
    const std::string& GetCharacterModelModelName();
    ...
    // Character model
    uint64_t m_characterModelAnimatorInstanceId = 0;
    uint32_t m_characterModelAnimationLayerIndex = 0;
    uint64_t m_characterModelSkinnedGameObjectId = 0;
