...
namespace Unloved {
    ...
    void Dobermann::Init(uint64_t id, DobermannCreateInfo createInfo, SpawnOffset spawnOffset) {
        ...
        skinnedGameObject->CreateRagdoll("Dobermann", filterData);
    ...
    void Dobermann::ResetToInitialState() {
        ...
        UpdateSkinnedGameObjectRotation();
        PlayAndLoopAnimation(AnimationSlot::DOBERMANN_LAY, 1.0f);
    }
    ...
    void Dobermann::UpdateMovement(float deltaTime) {
        ...
        if (Input::KeyPressed(HELL_KEY_T)) {
            if (Unloved::Player* player = Unloved::Session::GetLocalPlayerByViewportIndex(0)) {
                ...
                if (m_state == DobermannState::LAY) {
                    ...
                    PlayAnimation(AnimationSlot::DOBERMANN_LAY_TO_WALK, 1.0f);
                }
            }
        }
        ...
        if (m_state == DobermannState::GET_UP_FROM_LAY && IsAnimationComplete()) {
            ...
            PlayAndLoopAnimation(AnimationSlot::DOBERMANN_WALK, 1.0f);
        }
        ...
        if (m_state == DobermannState::WALK_TO_TARGET) {
            ...
            if (m_path.size() >= 2) {
                ...
                if (distanceToTarget < 0.2f) {
                    ...
                    PlayAnimation(AnimationSlot::DOBERMANN_STRETCH_TO_LAY, 1.0f);
                }
            }
        }
    }
    ...
    void Dobermann::PlayAnimation(AnimationSlot animationSlot, float speed) {
        SkinnedGameObject* skinnedGameObject = GetSkinnedGameObject();
        ...
        animatorInstance->PlayAnimation(m_animationLayerIndex, Bible::GetAnimation(AnimationProfile::DOBERMANN, animationSlot), speed);
    }

    void Dobermann::PlayAndLoopAnimation(AnimationSlot animationSlot, float speed) {
        SkinnedGameObject* skinnedGameObject = GetSkinnedGameObject();
        AnimatorInstance* animatorInstance = Animator::GetAnimatorInstanceByObjectId(m_animatorInstanceId);
        if (!skinnedGameObject) return;
        if (!animatorInstance) return;
        ...
        skinnedGameObject->SetAnimationModeToAnimated();
        animatorInstance->PlayAndLoopAnimation(m_animationLayerIndex, Bible::GetAnimation(AnimationProfile::DOBERMANN, animationSlot), speed);
    }
