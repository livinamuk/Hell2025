...
namespace Hell::Physics {
    ...
    uint64_t SpawnRagdoll(const glm::vec3& position, const glm::vec3& eulerRotation, const std::string& ragdollName, uint64_t parentObjectId, PhysicsFilterData filterData) {
        ...
        if (!asset) return 0;
        return SpawnRagdoll(position, eulerRotation, *asset, parentObjectId, filterData);
    }

    uint64_t SpawnRagdoll(const glm::vec3& position, const glm::vec3& eulerRotation, const RagdollAsset& asset, uint64_t parentObjectId, PhysicsFilterData filterData) {
