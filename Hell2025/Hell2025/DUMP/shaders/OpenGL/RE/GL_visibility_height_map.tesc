...
layout(location = 1) flat in int v_sceneRenderItemIndex[];

layout(location = 0) out vec3 tc_controlPointPosition[];
layout(location = 1) flat out int tc_sceneRenderItemIndex[];
...
float TessellationFactorAt(vec2 controlPosition) {
    ...
    // These are the four Terrain3D displacement-buffer strip coverages for
    // mesh_size 48: each density doubling halves the covered radius.
...
void main() {
    ...
    tc_sceneRenderItemIndex[gl_InvocationID] = v_sceneRenderItemIndex[gl_InvocationID];
