...
void main() {
    ...
    vec2 baseUv = controlPosition / vec2(max(heightMapMaximum, ivec2(1)));
    float terrainHeight = textureLod(u_HeightMapTexture, baseUv, 0.0).r;
    vec3 worldPosition = (u_modelMatrix * vec4(controlPosition.x, terrainHeight, controlPosition.y, 1.0)).xyz;
    worldPosition += GetTerrainDisplacement(controlPosition);
